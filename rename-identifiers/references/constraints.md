...
- **Don't rename a production export without updating every consumer.** A broken import is worse than a bad name. In diff mode, exported identifiers cannot be safely renamed — the diff does not show all consumers. Report the finding but mark it as **requires full-scope rename** and do not apply the rename.
