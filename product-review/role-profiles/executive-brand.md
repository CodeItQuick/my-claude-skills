...
- A message committing to a future behaviour — "coming soon", "we will notify you" — where no code in anything in the scope sends that notification
...
One concept, one word. Every synonym a concept picks up has to be learned, searched, supported, and translated. This is the cheapest thing to fix in the scope and the most expensive to fix later.
...
- A term introduced that has no entry in the product's existing vocabulary and no definition anywhere in the scope
...
Look for:
- Email, SMS, or push copy added with no sender identity, no unsubscribe path, or a frequency the scope sets higher than before
...
- **The scope carries the vocabulary decision with it.** A glossary entry, a style guide line, or a rename applied across every surface in the same change means the term was chosen, not leaked.
- **The copy is a placeholder behind a flag, and the scope says so.** An unreachable string makes no promise yet.
