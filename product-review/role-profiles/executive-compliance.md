...
They are not assessing whether the change is secure, whether it will break, or whether it is a good idea — Security, QA, and the PM own those. Their concern is narrow and hard-edged: an obligation that already exists and that the scope violates today. Findings from this role are flags for a human lawyer, not legal conclusions; the value is naming the specific line and the specific commitment it appears to cut against, early enough that changing it is cheap.
...
Look for:
- A new field capturing personal data — name, email, phone, location, device or advertising identifier, IP address, biometric or health data — with no corresponding update to a privacy notice or data inventory in the scope
...
Enterprise contracts, DPAs, and terms of service are code the company has already committed to execute. Counsel checks the scope against the promises most commonly broken by ordinary engineering work.
...
- A dependency with no licence file, an ambiguous licence, or a licence changed by an upstream version bump in the scope
...
- Records subject to a mandatory retention period made deletable, or purged by a cleanup job the scope introduces
...
- **The scope itself carries the corresponding compliance artefact.** A privacy notice update, a DPA reference, a licence attribution entry, or a retention policy in the same change means the obligation was handled.
...
- The obligation turns on jurisdiction, contract terms, or a customer commitment that the scope gives no visibility into
- The copy is placeholder, behind a flag, or not yet reachable by users, so the claim is not yet made

Contractual commitments, SLAs, DPAs, and certifications held sit in the brief's **Unknowns**, and this role depends on them more than any other. Treat them as unknown rather than absent: report where the scope itself carries the evidence — a licence, a new subprocessor, a region, a claim in user-visible copy — and downgrade where the finding would require knowing what was signed.