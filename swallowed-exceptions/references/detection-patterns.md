...
Patterns that frequently introduce swallowed exceptions. Each pattern is a *candidate*, not a finding — apply the evidence rules in `SKILL.md` and the suppression rules in `suppression-rules.md` before reporting.
...
Why it matters: the operation failed, the caller has no idea, and execution continues with potentially corrupt state.
...
Naming the binding `_` signals intentional discard, but without a comment explaining why proceeding is safe, it is a candidate. See suppression rule S3 for when a comment saves it.
...
The original error is swallowed; the replacement carries no cause chain. This makes root-cause analysis significantly harder. Flag when the original `e` is not passed to the replacement (e.g., `new Error("...", { cause: e })`).
...
// no "error" handler — unhandled errors on EventEmitter crash in Node ≤ LTS or silently vanish in some environments
```
...
## 10. `try/catch` wrapping an entire function with a blanket return
...
These look similar but are safe or out-of-scope:

- Catch that calls `throw e` or `throw new SpecificError("...", { cause: e })` — the error propagates with cause chain intact
- Catch that invokes a callback with the error: `callback(err)`, `reject(err)`, `next(err)`
- Catch in a test that asserts the error: `expect(() => fn()).toThrow()`
- Catch in a finalizer/cleanup path documented as best-effort (see suppression rule S4)
- `try/catch` wrapping code that is genuinely optional and the author has documented why failure is acceptable
- `process.on("uncaughtException", ...)` handlers that log and exit — those are intentional top-level boundaries